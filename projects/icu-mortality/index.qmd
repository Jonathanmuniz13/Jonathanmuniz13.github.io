---
title: "ICU Mortality: Interpretable Inference Meets Predictive Power"
description: "Two models on 91,713 ICU admissions: an interpretable logistic regression for clinical inference, and a cross-validated LightGBM (AUC 0.90) for prediction, with an honest accounting of why the two AUCs differ."
image: featured.png
date: 2026-05-04
categories: [Biostatistics, Machine Learning, Clinical Risk Modeling, R]
---
...
- **Logistic track.** A parsimonious 10 predictors, complete-case (71,926 rows), every coefficient interpretable. Lactate (about 75% missing) was dropped rather than imputed on weak ground, and I ran a complete-case selection-bias check. Retained versus dropped patients had near-identical mortality (8.5% versus 9.0%), which is what makes complete-case analysis defensible here rather than convenient.
- **LightGBM track.** 72 features including APACHE components and comorbidity flags, with no row-dropping, since gradient boosting routes missing values through its splits natively.
...
#| fig-width: 8
#| fig-height: 4.6
#| fig-cap: "Adjusted odds ratios per interquartile-range increase. Points left of the dashed line are protective; points right are associated with higher mortality. Scaling by IQR makes clinical effect sizes directly comparable across predictors on different units."
...
```{r}
#| include: false
ggsave("featured.png", p, width = 8, height = 4.6, dpi = 110, bg = "white")
```
...
- 📄 [Read the full report](Wids-ICU-Kaggle.pdf) covers the complete code, all exploratory figures, both models, CV results, and the Kaggle submission pipeline
- 💻 [Code on GitHub](https://github.com/Jonathanmuniz13/icu-mortality)
- 🗄️ Data: [WiDS Datathon 2020 (GOSSIS)](https://www.kaggle.com/c/widsdatathon2020)
